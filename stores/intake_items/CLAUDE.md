...
File via `stores intake add --invoker ai_autonomous` **whenever a local agent (planner, executor, code_reviewer, orchestrator) encounters substrate friction**. This is the primary path from P1 onward. The gatekeeper triages the item and routes it.
...
- `route --decision <D>` — gatekeeper routes; D ∈ {duplicate, needs_info, fast_track, normal_observation, arch_review_candidate, reject_noise}. `arch_review_candidate` routes to `routed`, creates an A### `architecture_reviews` row, sets `routed_to_arch_review`, and marks the downstream observation `pending_architecture_review=true` in the same transaction.
...
The schema enforces cross-row invariants via `required_when` and route-side validators:
...
3. `route --decision arch_review_candidate` creates the A### architecture-review side effect and refuses a router-minted observation that cannot be marked `pending_architecture_review=true`

Attempting a route variant without its required FK or side effect fails loud with a validation error.
