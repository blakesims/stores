...
| L176 | 🟢 T066 | self-reexec follow-up to L149: preserve daemon process by execing fresh binary after stale detection. Needs candidate-binary validation follow-up because a corrupted/stub global binary can otherwise be exec'd. |
| L181 | ⚪ T2 | `stores` CLI fail-silent: corrupted/stub binary returns exit 0 with empty stdout/stderr, violating substrate trust. |
| L182 | ⚪ T2/T3 | recurring global binary corruption: subagent/test `cargo install` overwrites `/home/blake/.cargo/bin/stores` with a 429K stub. Session SOP forbids subagent cargo-install; durable fixes are candidate validation (D) and private install path (C). |
...
1. **Candidate-binary validation before self-reexec (L182/D)** — do not exec a stale replacement unless `stores --help`/`--version` (or selfcheck) proves it is a real, non-stub stores binary; fail loud otherwise.
2. **Private substrate install path (L182/C)** — move runtime-owned binary away from global `~/.cargo/bin/stores` so subagents/operator installs cannot corrupt the daemon's launch path.
3. **T067/L178 handoff semantics** — finish closing the manual-drive ↔ daemon handoff gap so `in_review → wrap` does not stall outside active work.
4. **T070/L057 + T071/L058 + L059 observability** — durable invocation telemetry, edge throughput, transcript index; feeds future codex-as-subscriber and review automation.
5. **L151 auto-investigator / queue drain** — after status/review surfaces are trustworthy.
6. **Heart / Constitution / Architect governance** — intent-harden first; likely first substrate slice is ruling capture, not a full Heart store.
7. **L171/L172/L173 gatekeeper P3-P5** — deferred post-T053 follow-ups; surface after operator-trust layer is solid.
