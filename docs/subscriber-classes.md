...
| `auto_promote`              | observation_lifecycle | Registered in `dispatch_builtin`; driven by the `auto-promote` subscriber on `observations: confirmed → ready`. Mints a tasks row at `planning`; current auto-promoted rows land active so queued release can proceed, but the subscriber itself remains observation-side rather than task-combusting. |
...
| `cleanup_worktree`          | ceremony_post_accept  | Registered in `dispatch_builtin`; driven by terminal/post-integration cleanup wiring to remove safe terminal task worktree artifacts. Post-terminal repository ceremony; it does not start task work and should not be activation-gated.                            |
...
| `integrate`                 | work_starting         | Registered in `dispatch_builtin`; driven by the `integrate` subscriber on release edges into `integration_queued` (`accepted`, `complete`, `in_review`, and retry from `integration_blocked`). Drives the integration lane; gated on `activation == 'active'` (T140 P2) plus a schema guard on `start-integration`. |
...
| `release_to_integration`    | work_starting         | Registered in `dispatch_builtin`; driven by the `release-to-integration` subscriber after human acceptance or a human-ratified delegated policy releases wrapping work into the integration lane. Gated by durable acceptance policy.                                  |
| `schema_migrate`            | ceremony_post_accept  | Registered in `dispatch_builtin`; driven by the `schema-migrate` subscriber on `(integrated → integrated)` after `post_integration_step=cargo_installed`. Post-`integrated` repo-specific ceremony; activation already enforced upstream by the integration lane. |
| `user_escalation`           | safety_reconcile      | Registered in `dispatch_builtin`; invoked as the default `deployment_specialist` by failure paths such as deploy-blocked and silent-zombie handling rather than by legacy task-status subscriber edges. Always runs to file an escalation observation; never gated. |
