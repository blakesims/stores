...
| **ResourceLock** | DB-backed capacity/fencing primitive for exclusive shared resources (`main_branch`, deploy target, schema migration lane, shared test DB). Concrete state lives in `resource_locks`; operators use `stores resource-locks {acquire,release,list,recover-stale}` and framework callers use `handlers::resource_locks` with explicit `Actor` invokers. Locks carry owner, fencing token, TTL/recovery metadata, and transition-history audit rows. |
...
- **2026-05-10** — T144 landed the ResourceLock v1 substrate as concrete state: the `resource_locks` table via `SUBSTRATE_DDL`, `src/handlers/resource_locks.rs` acquire/release/check/recover helpers with explicit invokers, and audited `stores resource-locks` CLI verbs. `builtin:integrate` now uses that DB-backed state for `main_branch` mutation fencing.
