...
- **2026-05-10** — T144 landed the ResourceLock v1 substrate: `resource_locks` table via `SUBSTRATE_DDL`, `src/handlers/resource_locks.rs` acquire/release/check/recover helpers with explicit invokers, and audited `stores resource-locks` CLI verbs. `builtin:integrate` now uses that concrete substrate state for `main_branch` mutation fencing.
