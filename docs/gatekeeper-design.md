...
**Precondition:** evidence is sufficient, no duplicate dominates, **NO high-risk risk_flags are present**, and the cluster threshold has not crossed. Low-risk helper flags and normal product/bug flags may still route here. High-risk flags — every `touches_*` flag (`touches_actor_authority`, `touches_lifecycle`, `touches_subscriber_semantics`, `touches_runner_boundary`, `touches_schema_core`, `touches_dispatch_locks`, `touches_approve_token`, `touches_recovery_admin`, `touches_substrate_boundary`, `touches_deploy_ceremony`), plus `introduces_new_primitive`, `changes_boundary`, `security_sensitive`, `authority_surface_drift` — ALWAYS escalate to `arch_review_candidate` (decision 5) regardless of cluster count. Cluster threshold is for lower-risk repeated symptoms that individually look local.
