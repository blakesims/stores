...
pub fn harden_log_markdown(v: &Value) -> String {
    ...
    push_list_records(&mut out, "Decisions", obj.get("decisions"), &["id", "decision", "rationale", "source_quote"]);
    push_list_records(&mut out, "Scope cuts", obj.get("scope_cuts"), &["cut", "rationale", "source_quote"]);
    push_list_records(&mut out, "Alternatives rejected", obj.get("alternatives_rejected"), &["alternative", "why_rejected"]);
    push_list_records(&mut out, "Compress vs surface", obj.get("compress_vs_surface"), &["item", "judgment", "rationale"]);
...
fn push_list_records(out: &mut String, title: &str, val: Option<&Value>, keys: &[&str]) {
    let Some(arr) = val.and_then(|v| v.as_array()) else { return; };
    if arr.is_empty() { return; }
    out.push_str(&format!("### {title}\n"));
    for item in arr {
        if let Some(obj) = item.as_object() {
            let first = keys.iter().find_map(|k| obj.get(*k).and_then(|v| v.as_str()).map(|s| (*k, s)));
            if let Some((k, s)) = first {
                out.push_str(&format!("- **{k}:** {s}\n"));
            }
            for k in keys {
                if first.map(|(fk, _)| fk) == Some(*k) { continue; }
...
fn print_harden_log(v: &Value, indent: usize) {
    let pad = "  ".repeat(indent);
    let rendered = harden_log_markdown(v);
    if rendered.trim().is_empty() {
        return;
    }
    println!("{pad}harden_log:");
