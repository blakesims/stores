...
The token mechanism's purpose is to bind tier-A writes to this solo-operated host and to the AI honor-system. The threat is not malice; it's drift — an AI that gradually marks more rows `ai_with_human` because "the user is presumably available." The token makes drift fail-loud at the schema boundary while dropping per-turn user-presence crypto:

- **At rest:** `stores auth init` writes plaintext `~/.config/stores/approve.token` at mode 0600 plus `approve.token.hash`; it does not create an `.age` file. Host account permissions are the at-rest boundary.
- **Show is non-interactive.** `stores auth show` reads the plaintext file and prints it to stdout. It does not prompt, decrypt, shell out to `age`, or discover SOPS keys.
- **During a session, the AI may possess it.** The runtime protection is ask-first discipline: propose the row, wait for assent, attach the token to the proposed write. The token is not a per-turn proof of user presence.
