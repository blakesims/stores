...
fn git_workspace() -> tempfile::TempDir {
    ...
    std::process::Command::new("git").args(["config", "user.email", "t@example.com"]).current_dir(tmp.path()).output().unwrap();
    ...
    std::process::Command::new("git").args(["add", "README.md"]).current_dir(tmp.path()).output().unwrap();
    std::process::Command::new("git").args(["commit", "-m", "base"]).current_dir(tmp.path()).output().unwrap();
    tmp
}
...
fn insert_review(conn: &Connection, id: &str, status: &str, verdict: &str, head_sha: &str, attempt: i64) {
    conn.execute(
...
fn external_review_accept_precheck_t3_stale_head_pass_fails_loudly() {
    ...
    let ws = git_workspace();
    insert_task(&conn, ws.path(), "T3", "in_review");
    insert_review(&conn, "ER001", "passed", "PASS", "0000000000000000000000000000000000000000", 1);
    let err = accept(&schema, &conn).unwrap_err().to_string();
...
fn external_review_accept_precheck_t3_current_head_pass_succeeds() {
    ...
    let status: String = conn.query_row("SELECT status FROM tasks WHERE display_id='T900'", [], |r| r.get(0)).unwrap();
...
fn external_review_accept_precheck_tooling_failure_does_not_satisfy_accept() {
    let conn = Connection::open_in_memory().unwrap();
    let schema = install_db(&conn);
    conn.execute_batch("ALTER TABLE external_reviews ADD COLUMN held_reason TEXT").unwrap();
    let ws = git_workspace();
    insert_task(&conn, ws.path(), "T3", "in_review");
    insert_review(&conn, "ER777", "tooling_held", "TOOLING_FAILURE", &head(ws.path()), 1);
    conn.execute("UPDATE external_reviews SET held_reason='runner unavailable' WHERE display_id='ER777'", []).unwrap();
    let err = accept(&schema, &conn).unwrap_err().to_string();
    assert!(err.contains("ER777") && err.contains("TOOLING_FAILURE"), "{err}");
}
...
fn agents() -> AgentsYaml {
    AgentsYaml {
        agents: vec![AgentEntry { name: "external-review".to_string(), subscribes_to: vec![Subscription { store: "external_reviews".to_string(), transition: TransitionEdge { from: "".to_string(), to: "pending".to_string() }, predicate: None }], command: "builtin:external-review".to_string(), claim_window_secs: 300, retry_policy: RetryPolicy { max_attempts: 1, backoff: BackoffKind::Linear }, command_args: None }],
...
fn external_review_accept_precheck_revise_routes_executor_next_action() {
    ...
    let ctx = DispatchCtx { conn: &conn, agents: &agents, config_path: &cfg, policies_hash: "" };
    external_review::run(&row, &ctx).unwrap();
    let (_id, entry) = row::read_row(&schema, &conn, "T900").unwrap();
    assert_eq!(entry.get("status").and_then(|v| v.as_str()), Some("executing"));
