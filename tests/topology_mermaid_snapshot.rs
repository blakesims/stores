...
use stores::cli::topology::{Format, Opts, emit_mermaid};
...
fn ac2_5_mermaid_snapshot_matches() {
    ...
    let fixture_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/topology/expected.md");
...
fn ac2_3_starts_with_z0_and_has_workflow_state_diagram() {
    ...
    assert!(out.starts_with("## Z0"), "must begin with `## Z0` heading; got: {}", &out[..40]);
    // At least one stateDiagram-v2 block per workflow store (tasks):
    let count = out.matches("stateDiagram-v2").count();
    assert!(count >= 2, "expected at least 2 stateDiagram-v2 blocks (Z0 + Z2 tasks), found {count}");
    assert!(out.contains("## Z2: tasks workflow firing order"), "Z2 tasks workflow heading missing");
}
