...
use stores::cli::topology::{Format, Opts, emit_dot};
...
fn ac2_4_dot_snapshot_matches() {
    ...
    let fixture_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/topology/expected.dot");
...
fn ac2_1_three_zone_clusters_present() {
    ...
    assert!(out.starts_with("digraph stores_topology {"), "must start with digraph header");
    assert!(out.contains("subgraph cluster_z0_cross_store"), "Z0 cluster missing");
    assert!(out.contains("subgraph cluster_z1_tasks"), "Z1 tasks cluster missing");
    assert!(out.contains("subgraph cluster_z1_observations"), "Z1 observations cluster missing");
    assert!(out.contains("subgraph cluster_z1_gate"), "Z1 gate cluster missing");
    assert!(out.contains("subgraph cluster_z2_workflow_tasks"), "Z2 workflow_tasks cluster missing");
...
fn ac2_6_store_filter_z1_only_but_z0_complete() {
    ...
    assert!(out.contains("\"z0_observations\""), "Z0 must still show observations");
    assert!(out.contains("\"z0_gate\""), "Z0 must still show gate");
    assert!(out.contains("\"z0_tasks\""), "Z0 must show tasks");
    // Z1 only contains tasks
    assert!(out.contains("subgraph cluster_z1_tasks"), "Z1 tasks must be present");
    assert!(!out.contains("subgraph cluster_z1_observations"), "Z1 observations must be filtered out");
    assert!(!out.contains("subgraph cluster_z1_gate"), "Z1 gate must be filtered out");
}
...
fn ac2_8_no_icons_uses_text_codes() {
    ...
    assert!(out.contains("[label=\"A submit-plan\""), "expected A-prefixed text-code label: {out}");
    assert!(out.contains("H+ resume"), "expected H+ text-code for resume");
    // No Nerd Font glyphs (the four code points)
    for glyph in &["\u{f544}", "\u{f2b5}", "\u{f007}", "\u{f013}"] {
        assert!(!out.contains(glyph), "no-icons mode must not contain glyph U+{:04X}", glyph.chars().next().unwrap() as u32);
    }
}
