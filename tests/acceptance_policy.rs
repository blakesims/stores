...
fn conn() -> Connection {
    ...
    c.execute_batch(&ddl_for(&schema("external_reviews"))).unwrap();
    c
}

fn insert_task(c: &Connection, id: &str, policy: &str, review_policy: &str, decided: Option<&str>) {
...
fn row(c: &Connection, id: &str) -> Value {
    let mut stmt = c.prepare("SELECT * FROM tasks WHERE display_id=?1").unwrap();
...
fn ctx(c: &Connection) -> DispatchCtx<'_> {
    ...
    let cfg: &'static PathBuf = Box::leak(Box::new(PathBuf::from("/tmp/stores-acceptance-policy-test.yaml")));
    DispatchCtx { conn: c, agents, config_path: cfg, policies_hash: "" }
}
...
fn required_acceptance_cannot_be_delegated() {
    ...
    let err = dispatch_builtin("release-to-integration", &row(&c, "T501"), &ctx)
        .unwrap()
        .unwrap_err();
    assert!(err.to_string().contains("human acceptance required but not recorded"));
}

#[test]
fn delegated_by_policy_with_authoritative_task_review_releases() {
    let c = conn();
    insert_task(&c, "T502", "delegated_by_policy", "authoritative", None);
    insert_passed_review(&c, "T502");
    let ctx = ctx(&c);
    dispatch_builtin("release-to-integration", &row(&c, "T502"), &ctx).unwrap().unwrap();
    ...
    assert_eq!((lifecycle.as_str(), step.as_str(), by.as_str()), ("integration", "queued", "policy_delegate"));
}
...
fn ai_autonomous_cannot_change_acceptance_policy() {
    ...
    entry.insert("contract".into(), json!({"done_when":"x","scope_in":"y","scope_out":"z"}));
    entry.insert("human_acceptance_policy".into(), json!("optional"));
    let mut diff = BTreeMap::new();
    diff.insert("human_acceptance_policy".into(), json!("optional"));
    let errs = validate::validate(&s, &entry, Op::Update(diff), InvokerCtx::bare(Actor::AiAutonomous)).unwrap_err();
...
fn release_subscriber_registered_and_fires() {
    ...
    dispatch_builtin("release-to-integration", &row(&c, "T503"), &ctx).unwrap().unwrap();
    let (lifecycle, step): (String, String) = c.query_row(
        "SELECT lifecycle,integration_step FROM tasks WHERE display_id='T503'",
        [],
        |r| Ok((r.get(0)?, r.get(1)?)),
    ).unwrap();
    assert_eq!((lifecycle.as_str(), step.as_str()), ("integration", "queued"));
}
...
fn optional_policy_releases_without_acceptance() {
    ...
    let ctx = ctx(&c);
    dispatch_builtin("release-to-integration", &row(&c, "T504"), &ctx).unwrap().unwrap();
    let lifecycle: String = c.query_row("SELECT lifecycle FROM tasks WHERE display_id='T504'", [], |r| r.get(0)).unwrap();
...
fn delegated_policy_without_authoritative_review_does_not_write_delegate() {
    ...
    let ctx = ctx(&c);
    let err = dispatch_builtin("release-to-integration", &row(&c, "T505"), &ctx).unwrap().unwrap_err();
...
fn ai_with_human_token_can_change_acceptance_policy() {
    ...
    let mut diff = BTreeMap::new();
    diff.insert("human_acceptance_policy".into(), json!("optional"));
    validate::validate(&s, &entry, Op::Update(diff), InvokerCtx { actor: Actor::AiWithHuman, token_valid: true }).unwrap();
}
