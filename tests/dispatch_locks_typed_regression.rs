...
fn empty_policies() -> PoliciesYaml {
    PoliciesYaml { hash: String::new(), policies: vec![] }
}
...
fn agent(name: &str, store: &str, from: &str, to: &str, command: &str) -> AgentEntry {
    AgentEntry {
        ...
        subscribes_to: vec![Subscription {
            ...
            transition: TransitionEdge { from: from.to_string(), to: to.to_string() },
            predicate: None,
        }],
        command: command.to_string(),
        claim_window_secs: 300,
        retry_policy: RetryPolicy { max_attempts: 3, backoff: BackoffKind::Linear },
        command_args: None,
    }
}

fn agents(one: AgentEntry) -> AgentsYaml {
    AgentsYaml { agents: vec![one], deployment_specialist: None }
}

fn insert_history(conn: &Connection, store: &str, row_id: i64, display_id: &str, from: &str, to: &str) -> i64 {
    conn.execute(
...
fn insert_task(conn: &Connection, display_id: &str, status: &str, workspace_path: Option<&str>, drive_pid: Option<i64>) -> i64 {
    conn.execute(
        "INSERT INTO tasks \
         (display_id, status, title, slug, branch, workspace_path, drive_pid, contract, created_at, updated_at, created_by, updated_by) \
         VALUES (?1, ?2, 'typed regression', 'typed-regression', 'feat/typed-regression', ?3, ?4, \
...
fn l087_back_to_back_ratifies_stamp_ok_typed_postconditions() {
    ...
    let a = agents(agent("auto-promote", "observations", "confirmed", "ready", "builtin:auto-promote"));
    insert_ready_obs(&conn, "L087A");
    insert_ready_obs(&conn, "L087B");

    let dispatched = poll_once(&conn, &a, &empty_policies(), &cfg_path(&tmp), "l087", "epoch-l087").unwrap();
    ...
    let rows: Vec<(String, String, String, String)> = stmt.query_map([], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?)))
...
fn l107_zero_exit_without_convergence_demotes_to_error_via_postcondition() {
    ...
    let a = agents(agent("auto-scaffold", "tasks", "", "planning", "builtin:auto-scaffold"));

    let n = poll_once(&conn, &a, &empty_policies(), &cfg_path(&tmp), "l107", "epoch-l107").unwrap();
    assert_eq!(n, 1);
...
fn l116_seeded_rows_are_legacy_but_live_claim_is_try_claim_attempt_zero_same_epoch() {
    ...
    let a = agents(agent("l116-agent", "tasks", "ready", "in_review", "/bin/true"));
    ...
    let n = poll_once(&conn, &a, &empty_policies(), &cfg_path(&tmp), "l116", "epoch-l116").unwrap();
    assert_eq!(n, 1);
    ...
    assert_eq!(live_shape, ("try_claim".to_string(), 1, "epoch-l116".to_string()));
}
...
fn l122_silent_zombie_is_typed_and_retry_requires_next_retry_at() {
    let tmp = tempfile::tempdir().unwrap();
    let conn = fresh_db(&tmp.path().join("db.sqlite"));
    let row_id = insert_task(&conn, "T122", "planning", Some(tmp.path().to_str().unwrap()), None);
    ...
    assert_eq!(n1, 0, "terminal silent_zombie with NULL next_retry_at is neither watchdog-refired nor retried");
    ...
    assert!(matches!(terminal_reason.as_str(), "error" | "silent_zombie"));
    assert!(attempt >= 2, "elapsed next_retry_at must move the row beyond its original attempt; got {attempt}");
}

#[test]
fn l141_auto_drive_ok_only_after_drive_pid_postcondition_passes() {
    let tmp = tempfile::tempdir().unwrap();
    let conn = fresh_db(&tmp.path().join("db.sqlite"));
    let row_id = insert_task(&conn, "T141", "planning", Some(tmp.path().to_str().unwrap()), None);
...
fn investigator_repeated_polls_keep_single_dispatch_lock_ok() {
    ...
    assert_eq!(n2, 0, "second poll does not redispatch the same transition attempt");
...
fn investigator_duplicate_transition_history_after_terminal_state_does_not_double_spawn() {
    ...
    assert_eq!(
        poll_once(&conn, &agents, &empty_policies(), &cfg_path(&tmp), "dup", "epoch-dup")
            .unwrap(),
        1
    );
