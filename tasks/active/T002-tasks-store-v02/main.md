...
- **Reviewer:** code-reviewer agent
- **Cycle:** 2 of max 3
- **Issues:** 0 critical / 0 major / 0 minor (all 6 cycle-1 items closed)
- **Status next:** EXECUTING_PHASE_8
- **Summary:** All six cycle-1 items resolved and verified at source, test, and live-smoke layers. **M1 (files_changed CSV→array):** `compute_submit_execute` (submit.rs:729-737) now splits on comma, trims, drops empties, and stores `Value::Array(Vec<Value::String>)`. Confirmed at the DB layer via `sqlite3 .stores/db.sqlite "SELECT cycles ..."` after a real `submit-execute --files-changed "src/foo.rs,src/bar.rs"`: `"files_changed": ["src/foo.rs", "src/bar.rs"]` (proper JSON array). Rendered main.md shows `**Files:**\n  - \`src/foo.rs\`\n  - \`src/bar.rs\`` — the `{{#each}}` iteration now works. Two new tests (`m1_files_changed_stored_as_json_array`, `m1_files_changed_trims_whitespace_and_drops_empties`) assert `is_array()`, length, and exact contents including whitespace-stripped/empty-dropped semantics. **M2 (`at` timestamps):** verified at all three insertion sites — submit.rs:563 (plan_review_log), submit.rs:724 (executor), submit.rs:886 (review). Each calls `now_iso8601()`. Three new tests (`m2_plan_review_log_entry_has_at_timestamp`, `m2_executor_entry_has_at_timestamp`, `m2_review_entry_has_at_timestamp`) assert `at.is_some()` and ISO-8601 shape (`'T'` and `'-'`). Live smoke confirms rendered main.md's three `**At:** 2026-04-26T17:16:56Z` entries. **m1 (executor-brief CLI-only):** template lines 71-74 now read `stores tasks submit-execute … --commit <sha> --files-changed "a.rs,b.rs"` followed by `stores tasks render` and an explicit `**Do NOT edit main.md directly.**`; line 95 routes BLOCKED through `submit-execute --summary "BLOCKED: <reason>"`. No mention of editing main.md or setting Status manually anywhere in the template. **m2 (4 carry-forward unit tests):** all four landed at submit.rs:2379-2634 with the contracted names: `ac7_p5m2_open_questions_appended_to_plan_review_log_entry` (asserts 2 questions in the array), `ac7_p5m3_submit_targets_consulted_for_field_lookup` (constructs a custom schema with `submit_targets: {submit-execute: my_exec_log}` and asserts the entry written to `my_exec_log` rather than `cycles` — proves the lookup fires, not just the fallback), `ac7_p5m4_review_summary_and_details_separate_keys` (distinct strings to `--summary` and `--details-from-file` end up at `review.summary` and `review.details` respectively, with `assert_ne!` on the values), `ac7_p6m2_bundled_sentinel_routes_to_in_memory_template` (loads planner template from `BUNDLED_STORE_TEMPLATES`, builds context, calls `render_template`, asserts "Methodical and thorough" persona text in output — the in-memory template path used by `brief::compute` after sentinel detection). **m3 (framework-actor filter):** dynamic.rs:183-186 — `if transition.actor == Some(crate::schema::actor::Actor::Framework) { continue; }` placed before the BASE_VERBS check; `stores tasks --help` output verified live (no `start` verb listed); `grep -i 'start'` on `--help` returns no match. **m4 (README 29 lines):** `wc -l` = 29, under the 30-line limit. **Test totals:** 297 unit tests pass (288 prior + 9 new), 0 failed, 0 ignored. **Live smoke (executed by reviewer in `/tmp/tmp.5OFkdGm9hK`):** `init` → `install tasks` → `add --invoker ai_with_human` → `submit-plan` → `submit-plan-review READY` → `submit-execute --files-changed "src/foo.rs,src/bar.rs"` → `submit-review --gate PASS` → `render` all green; final `tasks/completed/T001-test-task/main.md` byte-clean and contains the proper Files list and three `**At:**` ISO-8601 timestamps. **No drift:** `git diff 84b6385..HEAD --stat` shows changes confined to `src/cli/dynamic.rs` (+4 LOC), `src/handlers/submit.rs` (+403/-17 — fixes + tests), `stores/tasks/README.md` (+4/-11), `stores/tasks/templates/executor-brief.md.tpl` (+5/-10), and the task's own `main.md`. No unrelated edits. Commit hygiene clean: 4 functional commits (`27a4302` M1+M2, `aaf2717` m1, `bab3e93` m3, `8891210` m4) + 1 log commit (`caa7c84`); no amends, no force-push.
- **What's good:** Reviewer's required-action labels were honored 1:1 (M1 → `m1_*`; M2 → `m2_*`; carry-forwards → `ac7_p5*` / `ac7_p6m2_*`); the M2 fix lands at exactly the line numbers cycle-1 specified (submit.rs:564, :723, :879 → :563, :724, :886 post-edit shifts); the M1 trim+filter ordering matches the `open_questions` list-construction precedent so future readers see one canonical pattern; the m3 fix is generic (any future framework-actor transition is hidden automatically, not just `start`); the m2 ac7_p5m3 test deliberately constructs a non-canonical-named schema to prove the `submit_targets.get(verb)` lookup actually fires — the cycle-1 critique that hardcoded fallbacks masked the lookup is directly answered.
- **Carry-forward to Phase 8 (binding):** None. All cycle-1 items closed.
- → Details: `code-review-phase-7.md`
...
- **Reviewer:** code-reviewer agent
- **Cycle:** 1 of max 3
- **Issues:** 0 critical / 0 major / 3 minor (informational)
- **Status next:** EXECUTING_PHASE_9
- **Summary:** All four ACs verified at source/test/install layers. AC8.1 PASS — live install in fresh tmp dir produces `.claude/skills/tasks:start/SKILL.md` byte-identical to the bundled source (`diff -q` empty); the `tasks_start_install_byte_identical` Rust unit asserts the same. AC8.2 PASS — grep for the 10 allowed `stores tasks` verbs returns 22 matches; grep for forbidden verbs (`transition`, `update`, `init`, `install`, `resume`, `start`) returns 0; grep for forbidden references (CodeRabbit, merge-review*, phase-review*, pi-extension, MERGE_READY, MERGE_REVIEW) returns 0. AC8.3 PASS — `python3 -c "yaml.safe_load(...)"` on the frontmatter returns the expected 5-key dict (`name`, `description`, `user_invocable`, `requires_stores`, `effort`). AC8.4 PASS — literal `## DONE_WHEN propagation rule` heading at line 340; DONE_WHEN appears 10 times; every Task subagent spawn block contains an explicit `> **DONE_WHEN:** {…}` callout (4 occurrences across the 4 stages). Skill structure matches the plan: Stage 0 context gate / Stage 1 Intent Contract + add / Stage 2 plan / Stage 3 plan-review / Stage 4 plan gate / Stage 5 phase loop with 5c gate table covering PASS-not-last, PASS-last, REVISE, and 4th-REVISE-auto-BLOCKED. Stage 6 (CodeRabbit) and Stage 7 (Completion) explicitly absent per plan. After every submit, prose explicitly calls `stores tasks render <id>` (4 explicit lines + 1 reminder) — main.md never goes stale during the orchestrator loop. The sibling `task:next` skill was correctly de-stubbed: uses `stores tasks list --status ready --limit 1 --sort updated_at` to find work, hands off via `Task(subagent_type="tasks:start", …)` or `/tasks:start <id>`. `BUNDLED_SKILLS` registration in `src/cli/skills.rs:25-28` is clean; `all_skills_bundled` test renamed from `all_four_skills_bundled` and asserts count == 5 + each name explicitly. **Tests:** 298 unit tests pass (296 prior + 2 new), 0 failed, 0 ignored. **e2e:** `bash tests/e2e.sh` exits 0 on all 13 steps. **Commit hygiene:** 4 functional commits (`05233b6` 8.1, `2c66e8d` 8.3, `eaf576c` 8.4, `98fe759` log), 1 task per commit, no amends, no force-push.
- **What's good:** Verb hygiene is exact — 10 allowed verbs, 0 forbidden verbs, 0 forbidden references, no overshoot even in passing prose. DONE_WHEN propagation discipline is enforced inline: every Task spawn block literally contains a "**Always include in the … prompt:** > **DONE_WHEN:** {…}" callout (lines 146-148, 179-181, 232-234, 269-271). "Do not edit main.md directly" is explicit in Non-negotiable rules (line 29-30) — `stores tasks render <id>` is the single write path. The 5c gate table covers the 4th-REVISE → BLOCKED path with the right framing ("Framework sets status `blocked` automatically — surface `blocked_reason` to user"), matching the schema-level guard. The new `tasks_start_install_byte_identical` test mirrors AC8.1 wording exactly — not just "did install run" but "is the file byte-identical." `task:next` correctly drops the v0.1 observations fallback and uses only `list`/`show` from the allowed verb set; it avoids inventing the non-existent `stores tasks start` verb (which would also violate AC8.2's allowlist).
- **Minor (informational) carry-forwards — non-binding:** (m1) Skill prose under-documents legal gate values: `submit-plan-review` accepts `READY|NEEDS_WORK|NOT_READY` but skill shows only `READY|NEEDS_WORK`; `submit-review` accepts `PASS|REVISE|FAIL` but skill shows only `PASS|REVISE`; the 5c gate table omits FAIL. An orchestrator following the skill literally cannot trigger NOT_READY (abandon-plan) or FAIL (abandon-phase), even when the underlying agent recommends them. AC8.2 doesn't require enumerating gate values, only no-out-of-allowlist verbs — which holds. Suggest a clarifying line in each section. (m2) Plan task 8.4 specified `stores tasks start <id>` as the verb to invoke `tasks:start` from `task:next`; the executor (correctly) used `Task(subagent_type="tasks:start", …)` instead because `stores tasks start` is a framework-internal verb hidden from the public CLI surface (per Phase 7 m3) and would violate AC8.2's allowlist. The Execution Log "Deviations" section says "None" — should be amended to record this necessary deviation. (m3) The skill spawns `task-workflow:planner|plan-reviewer|executor|code-reviewer` subagents but no frontmatter declares the `task-workflow` plugin dependency. `requires_stores: [tasks]` is present (per plan 8.2) but is documentation-only for v0.2; a future `requires_plugins` field could declare the plugin dependency in v0.3. One-line note in Non-negotiable rules would help.
- **Carry-forward to Phase 9 (binding):** None. The three m-items above are non-binding polish suggestions; Phase 9 proceeds. If Phase 9's smoke test surfaces a real orchestrator gap from the missing NOT_READY/FAIL gate enumeration, m1 graduates to a real fix.
