...
**Summary:** AC2.8 and AC2.9 are both implemented correctly and the live haiku-smoke pathology (markdown-fenced planner envelope in `final_message` with `structured_output: None`) is demonstrably recovered by Layer 2 (SAP). The `parse_envelope` signature matches spec exactly (`fn parse_envelope(out: &RunnerOutput, agent_role_normalized: &str) -> Result<(AgentEnvelope, &'static str)>`); the layer order is sdk → sap → legacy with clear section comments; the schema lookup uses `BUNDLED_AGENT_SCHEMAS.iter().find(...).and_then(...)` returning `Option<Value>` and falls through cleanly when the role is unknown or the schema can't parse; the all-layers-fail error explicitly lists the layers attempted; and drive's submit eprintln correctly interpolates `source={source_tag}`. Phase boundary discipline is observed: only `src/handlers/drive.rs` was changed (plus the task doc); no Cargo version bump, no schema or runner changes, no `AgentEnvelope` shape change, and the AC2.7 retry-exhausted surface is preserved verbatim at lines 443-455. `cargo build --features runner-claude-code` succeeds; `cargo build` (no features) succeeds; `cargo test --features runner-claude-code` reports 386 unit + 2 integration = 388 passed, 0 failed (matches the executor's claim, all 4 new tests green); after rebuilding the on-PATH binary against the current code, `bash tests/drive_e2e.sh` passes both AC7.1 and AC7.1b with `source=sap` correctly logged on every submit (mock fixtures' plain-JSON `final_message` is recovered cleanly by SAP's no-schema branch, which is acceptable per the executor's deviation note 1). However, **`cargo test` (no features) regresses with 4 failures** — 2 inherited from Phase 1 Revise (`runner::sap::tests::sap_validates_against_schema`, `runner::sap::tests::sap_recovers_planner_envelope_from_haiku_transcript`) and **2 newly introduced by Phase 2 Revise** (`handlers::drive::tests::parse_envelope_source_tag_sap_layer` and `handlers::drive::tests::three_layer_fallback_for_markdown_fenced_planner_output`). Root cause: `src/runner/sap.rs:200-205` stubs `validate_against_schema` to return `false` under `#[cfg(not(feature = "runner-claude-code"))]`, and Phase 2 Revise's two new SAP-layer drive tests use role `"executor"` / `"planner"`, which `BUNDLED_AGENT_SCHEMAS` resolves to a real schema — SAP then rejects every candidate (validation always fails) and the layer chain falls through to Layer 3, which has nothing to scan because `stdout` is empty. The original Phase 2 baseline review's AC2.2 ("`cargo test` (default features) passes") is therefore regressed. **Phase 3 unblock:** the live haiku smoke uses `--features runner-claude-code` (per Phase 3's Smoke command sequence), which exercises the real `jsonschema` runtime, so this regression does NOT block Phase 3 attempt 2. The DONE_WHEN scenario is reachable. The fix is small (≤30 lines): either feature-gate the two new drive tests with `#[cfg(feature = "runner-claude-code")]`, or change the SAP stub to skip schema validation (return `true` or unconditionally accept the candidate when the feature is off, restoring no-feature parity). A minor secondary issue: `parse_envelope_source_tag_sdk_layer` sets `structured_output_source: Some("sdk")` on the input `RunnerOutput` (line 1331), but `parse_envelope` does not consume that field — the source tag is derived solely from which layer fired. Cosmetic, non-blocking; could be `None` for clarity.

**AC verification:**
- **AC2.8 (3-layer parser): ✅** `src/handlers/drive.rs:550-611`. Signature `fn parse_envelope(out: &RunnerOutput, agent_role_normalized: &str) -> Result<(AgentEnvelope, &'static str)>` (line 550) matches spec. Layer 1 SDK at lines 552-557 (returns `(envelope, "sdk")`). Layer 2 SAP at lines 560-579: looks up schema from `BUNDLED_AGENT_SCHEMAS` via `.iter().find(...).and_then(...)` (565-566), passes `schema_value.as_ref()` to `crate::runner::sap::extract_envelope_from_text` (568-571), returns `(envelope, "sap")` on Some. Layer 3 Legacy at lines 581-611: tries `final_message` direct parse first (582-588), then last-line stdout scan (591-611). All-layer-fail error message at 597-600 lists "layers attempted: sdk, sap, legacy". Test `three_layer_fallback_for_markdown_fenced_planner_output` (lines 1261-1311) constructs the Phase-3-attempt-1 pathology shape and asserts SAP recovers `AgentEnvelope::Planner` with `source == "sap"`. Passes under `--features runner-claude-code`.
- **AC2.9 (source tag in eprintln): ✅** `src/handlers/drive.rs:516-518` — submit-success eprintln now reads `[{display_id}] phase {current_phase} cycle {current_cycle}: {agent_role} → submitted (gate={gate_display}; source={source_tag})`. The `source_tag` is destructured from `parse_envelope`'s return at line 477. Three new tests at lines 1318-1374 cover sdk/sap/legacy paths (`parse_envelope_source_tag_sdk_layer`, `parse_envelope_source_tag_sap_layer`, `parse_envelope_source_tag_legacy_layer`); the first and third pass under both feature configurations, but the **sap variant fails under `cargo test` (no features)** — see Findings.

**Findings:**
- **[minor] `cargo test` (no features) regresses with 2 newly-failing tests.** `handlers::drive::tests::parse_envelope_source_tag_sap_layer` (`src/handlers/drive.rs:1337-1352`) and `handlers::drive::tests::three_layer_fallback_for_markdown_fenced_planner_output` (`src/handlers/drive.rs:1261-1311`) both fail with `all 3 parse layers failed (layers attempted: sdk, sap, legacy)`. Root cause: under `#[cfg(not(feature = "runner-claude-code"))]`, `src/runner/sap.rs:200-205` returns `false` from `validate_against_schema`, so SAP rejects every candidate when a schema is supplied; both new tests use roles (`"executor"` / `"planner"`) that resolve to a real schema in `BUNDLED_AGENT_SCHEMAS`, and both have empty `stdout`, so Layer 3 has nothing to scan. (Phase 1 Revise's `sap_validates_against_schema` and `sap_recovers_planner_envelope_from_haiku_transcript` already had this issue; Phase 2 Revise inherited it and added 2 more failures.) **Remediation (pick one):** (a) gate the two new drive tests with `#[cfg(feature = "runner-claude-code")]` (~4 lines, surgical, preserves test coverage under the feature where it matters); (b) change the SAP no-feature stub at `src/runner/sap.rs:200-205` from `false` to a path that accepts the candidate without validation (e.g., return `true` and document that no-feature builds skip schema validation), which would also fix the 2 inherited Phase 1 Revise failures. Option (b) is the more correct architectural fix; option (a) is the minimal change that satisfies AC2.2 for Phase 2 Revise's own tests.
- **[minor — non-blocking, cosmetic]** `parse_envelope_source_tag_sdk_layer` (`src/handlers/drive.rs:1331`) sets `structured_output_source: Some("sdk")` on the input, but `parse_envelope` derives the source tag itself from which layer fired and ignores the input field. Consistency-wise this is harmless but could be `None` like the other tests for clarity.
