...
- **Gate:** PASS (cycle 2) — was REVISE in cycle 1 (one major data-loss bug); M1 fixed cleanly, advance to Phase 5
- **Reviewed:** 2026-04-26 by `code-reviewer` (cycle 1), 2026-04-26 by `code-reviewer` (cycle 2)
- **Commit:** `8beeb67` (cycle 1) → `0ba36d1` (cycle 2 fix)
- **Issues:** 0 critical / 1 major / 5 minor (cycle 1) → 0 critical / 0 major / 5 minor (cycle 2)
...
- **Commit:** `0ba36d1` — fix(T001 phase 4): deep-merge Record updates to preserve sibling sub-fields

**Cycle 2 review verdict (2026-04-26):** **PASS** — advance to Phase 5.
- **M1 status:** Resolved. Reviewer ran live repro against `kitchen_sink` fixture: `add --notes "keep-me" --severity info` → `K001` → `update K001 --severity warning` → DB now holds `{"notes":"keep-me","severity":"warning"}` (notes preserved). Test `update_record_subfield_preserves_siblings` passes (42/42 total). Schema-driven `is_record` check at `update.rs:46-48` is correct; type-mismatch guard `(Some(Value::Object(_)), Value::Object(_))` falls through safely to wholesale insert if either side isn't an Object — no panic surface. SQL writer split (Record vs List) correctly serializes `merged.get(field.name)` for Records and `new_val` for Lists, matching the Decision Matrix entry.
- **Decision Matrix entry:** Verified — line 273 of main.md adds the row "Record vs List update merge semantics" with the `(a) deep-merge Record, replace-wholesale List/scalar` decision.
- **Scope discipline:** PASS. Fix commit `0ba36d1` touches only `src/handlers/update.rs` (+98 LOC: ~17 in merge/writer + test module), `tasks/active/T001-stores-framework-v01/main.md` (Decision Matrix + Revise cycle 1 sub-block), and `tasks/global-task-manager.md` (status sync). No silent fixes for m1–m5 — Integer→0 coercion at update.rs:108-114 is byte-identical to cycle 1. No unrelated files touched.
- **Forward-compat:** Phase 6's `transition` handler (which will share update's merge shape) now has a clean reference implementation to inherit; the `is_record` schema lookup and the Object-Object guard pattern transfer directly.
...
- **Commit:** `ebd667b`
- **Issues:** 0 critical / 0 major / 4 minor
- **Summary:** All 8 Phase 5 ACs verified end-to-end. `cargo test` 79/79 pass (matches executor claim — 35 new validation tests). Live E2E in fresh tmp dir against `kitchen_sink`: (AC2/AC3) `add --title hi --verdict T3` (no contract) → fails citing `contract.done_when`/`contract.scope_in`/`contract.scope_out` with the rule `(because triage.verdict == 'T3')`; `add --title T1demo --verdict T1` succeeds; `add --title T3demo --verdict T3 --done-when X --scope-in Y --scope-out Z` succeeds — cross-Record `required_when` resolves correctly through the reassembled nested EntryMap. (AC4) `add --slug "Bad Slug!"` is rejected with `value 'Bad Slug!' does not match pattern '^[a-z0-9-]+$'`; valid slugs pass. (AC5) The kitchen_sink fixture has no `actor: human` field, but a custom `probe_store` with `answer` field marked `actor: human` under `CLAUDECODE=1` produces the **exact** documented error format: `"field 'answer' requires actor 'human'; invoker is 'ai_autonomous' (auto-detected from $CLAUDECODE; pass --invoker human to override if appropriate)"`; adding `--invoker human` clears it and the row writes. (AC6) Single `add` with missing title + bad slug + bad priority + verdict=T3 returns 6 errors aggregated in one pass, alphabetically sorted by `field_path`. (AC7) Phase 2 M1 fix verified — `required_when::contains_keyword` is genuine token-aware boundary matching, regression tests `parse_accepts_quoted_or_in_literal` and `parse_accepts_quoted_and_in_literal` both pass. (AC8) 79 tests pass, no flakes. The validator's `lookup` walks dotted paths through nested `Value::Object`, returning `None` silently on missing segments — `required_when` whose `lhs_path` references a non-existent field stays silent rather than crashing. The `Op::Transition(verb)` shape passes verb name to `actor::check_transition_actor`; the negative case is exercised in `actor.rs::transition_actor_mismatch_fires` (the `mod.rs::transition_actor_mismatch_caught` test name is misleading — it actually verifies `ai_with_human` accepts both, not the rejection path; not a defect, just nomenclature). All 4 executor deviations are defensible: D1 (`Pattern { pattern }` carries the offending pattern in errors — improvement), D2 (`--invoker` flag pulled into Phase 5 — needed for AC5 live demo, doesn't preempt Phase 6's transition wiring since it's `global(true)` in clap and inherits cleanly), D3 (fixture rename `triage.notes` → `triage_notes` is the framework correctly enforcing leaf-arg uniqueness at install — defensible), D4 (alphabetical sort assertion correction — trivial).
  - **4 Minors:** (m1) `is_absent` in `check_required` only checks `None || Value::Null`; an empty string `--done-when ""` satisfies a `required_when` rule (verified live: T3 with `--done-when "" --scope-in "" --scope-out ""` writes successfully). v0.1 demo doesn't trigger this; tighten in a future phase if needed. (m2) `mod.rs::transition_actor_mismatch_caught` is misleadingly named — it tests the positive `ai_with_human` accepts-both case, not a transition-actor mismatch. The negative case is in `actor.rs::transition_actor_mismatch_fires`; rename for clarity. (m3) Reserved-column-name leaf collision (carried from Phase 2 m5 / Phase 3 m2 / Phase 4 m5) still not caught at install time — Phase 4's `is_reserved` list in `dynamic.rs` is not mirrored at install, so a user leaf named `status` would surface as SQLite's `duplicate column name`. Phase 5 didn't pick it up; deferrable. (m4) `Schema.default_actor` field is now consumed (`mod.rs:91`, `mod.rs:53/58`) — closes the m3 carried from Phases 2/3 ("YAGNI" concern is now resolved by actor-default fallback in `effective_actor`).
  - **Forward-compat for Phase 6 (lifecycle transitions):** Validator's `Op::Transition(String)` API is stable; `validate(schema, &EntryMap, Op::Transition(verb), invoker)` gives Phase 6's transition handler exactly what it needs (verb to look up the transition row in `schema.lifecycle.transitions`, the merged entry-diff for field-level rule firing, the resolved invoker for actor checks). Transition-actor enforcement is already wired via `actor::check_transition_actor` (called from `mod.rs:46`); Phase 6 just needs to (a) gate state-machine `from`→`to` legality (validator does NOT check this — it's Phase 6's job to verify current `status == transition.from`), (b) merge the entry diff with the read row before calling validate (same shape as `update.rs:42-62`), (c) write the new `status` to the DB. The `--invoker` flag is `global(true)` in clap so transition subcommands inherit it for free.
  - **Forward-compat for Phase 7 (gate store):** The `gate.answer` field with `actor: human` is exercised by the live `probe_store` test above — the actor enforcement engine is ready as-is. The `transition: answer` with `actor: human` will be caught by `check_transition_actor` (`actor.rs:50`) without further changes. No additional work required in the validator for Phase 7.
...
### Phase 7: Bundled `gate` store + human-only actor enforcement demo (Code Review)

- **Gate:** PASS
- **Reviewed:** 2026-04-26 by `code-reviewer`
- **Commit:** `e0f1dac`
- **Issues:** 0 critical / 1 major / 3 minor
- **Summary:** All 6 Phase 7 ACs verified end-to-end against a fresh tmp dir with both bundled stores installed. `cargo test` 83/83 pass (matches executor claim — no new tests, see m2). Live E2E: (AC1/#3) `stores install ./stores/observations` then `stores install ./stores/gate` → both registered in `manifest.yaml`; `sqlite3 .stores/db.sqlite ".tables"` → `gate observations`; multi-store coexistence in one DB confirmed. (AC2/#9) `stores gate add --type decision --question "Soft or hard delete?" --options "soft|hard" --task-ref L001` → `G001`, status=`pending`, `options=["soft","hard"]`. (AC3/#10) `stores gate answer G001 --answer hard --invoker human` → `Transitioned G001: pending → answered`; row reads back `answer=hard`. (AC4/#11) `CLAUDECODE=1 stores gate answer G002 --answer a` (no `--invoker`) → exit 1 with **two** errors (defense in depth — both layers fire): the transition-actor check (`<transition:answer>: transition 'answer' requires actor 'human'; invoker is 'ai_autonomous'…`) AND the field-actor check on `answer` (`field 'answer' requires actor 'human'; invoker is 'ai_autonomous' (auto-detected from $CLAUDECODE; pass --invoker human to override if appropriate)`). Both messages cite the field/transition `answer`, the required actor `human`, the `$CLAUDECODE` source, and the `--invoker human` override — exactly the documented format. (AC4b) `CLAUDECODE=1 stores gate answer G002 --answer a --invoker human` → succeeds. (AC5) `CLAUDECODE=1 stores gate cancel G003` (no `--invoker`) → `Transitioned G003: pending → cancelled` — cancel transition's declared actor `ai_autonomous` matches env-detected invoker; the field-level actor check on `answer` correctly does not fire because `answer` is not in the diff (this is exactly what framework fix #2 enables). DONE_WHEN #3 (fully), #9, #10, #11 all close. Phase 8 forward-compat verified empirically: `SELECT o.display_id, o.status, g.display_id, g.status, g.answer FROM observations o LEFT JOIN gate g ON g.task_ref = o.display_id WHERE o.display_id = 'L001';` returns `L001|triaged|G001|pending|` — real JOIN match with non-NULL gate display_id, satisfying Phase 8 AC #2 as-is.
  - **Framework fix #1 (`Value::Null` treated as absent in `check_actor`, src/validate/actor.rs:23-27):** Defensible. Symmetric with `required.rs:43` (`is_absent = field_value.is_none() || field_value == Some(&Value::Null)` since Phase 5). `read_row` injects `Value::Null` for absent columns (`row.rs:201, 215`) so without this symmetry, an unset optional field trips actor enforcement spuriously on operations that don't write it. `check_actor` has only one call site (`mod.rs:105`); `check_transition_actor` takes a verb name not a field-path lookup so the Null question doesn't apply there — no other call sites need parallel treatment. The 83-test suite still passes; no existing test asserted actor enforcement on a `Value::Null` field (which would have been wrong anyway). Minimal correct fix.
  - **Framework fix #2 (`Op::TransitionWithDiff` scoping, src/validate/mod.rs + src/handlers/transition.rs):** Defensible — and necessary. Pre-fix the validator scoped actor enforcement to the full *merged* row, so a `human` invoker calling `answer` was re-validated against `type/question/options/task_ref` (fields the `ai_autonomous` add-time invoker wrote — none of which carry `actor: human` so didn't actually error in this direction); the live failure was the reverse: AI cancelling a gate where a human had written `answer`, which spuriously failed because the field-level actor check fired on `answer` even though cancel doesn't touch it. Live AC5 confirms the fix unblocks this. Op semantics verified: `Op::Add` correctly passes the entry as both required-entry and actor-entry (in Add, the entry IS the diff). `Op::Transition(verb)` (no diff) falls back to `entry` for actor scoping at `mod.rs:48` — currently no caller emits this; reachable only via direct unit test (`mod.rs:350-351`); semantically equivalent to old behaviour, correct as-is for tests. Naming nit: `Op::Transition` becomes vestigial — Phase 8 or v0.2 could fold variants into `Op::Transition { verb, diff: Option<EntryMap> }`; not gate-blocking.
  - **1 Major (M1) — `Op::Update` has the same merged-row bug, unfixed.** The executor's own diagnosis ("actor checks should apply to writes, not to reads of pre-existing data") logically applies to `Op::Update` too. `handlers/update.rs:65` passes `&merged` to the validator with `Op::Update`; `validate()` then uses that merged entry for actor scoping at `mod.rs:50`. **Live repro:** `gate add --type decision --question "Q?" --options "a|b"` (G001 by ai_autonomous) → `gate answer G001 --answer a --invoker human` (human writes answer=a) → `CLAUDECODE=1 stores gate update G001 --question "Q?-updated"` (AI tries to fix the question — no actor constraint on `question`) → `Error: validation failed: - answer: field 'answer' requires actor 'human'; invoker is 'ai_autonomous'…`. Same shape, identical root cause, identical fix pattern (~10 LOC: `Op::UpdateWithDiff(diff)` mirroring transition's switch). **Not gate-blocking for Phase 7** because the demo path doesn't traverse it AND Phase 8's e2e per plan lines 244-255 doesn't update an `answer`-bearing gate row by a different invoker. **Recommendation:** Address in Phase 8 alongside the e2e + README — single-PR cleanup. If Phase 8 keeps moving and this slips, capture as v0.2 and add a regression test in v0.1 that pins the current (buggy) behaviour so we know what we're carrying.
  - **3 Minors:** (m1) `task_ref` declared as `text` in the gate schema, not `display_id` as the plan called for at line 236 — schema description string acknowledges the cross-store-ref intent, but the Execution Log block doesn't list this as a deviation. Add a "Deviations" sub-bullet to the Phase 7 entry. Functionally harmless — `display_id` and `text` both write as TEXT. Per the prompt, either is acceptable. (m2) Phase 7 added two genuine framework changes but no unit test for either. The Null-as-absent fix has no regression test (a `Value::Null` case in `actor.rs::tests` would be ~5 LOC). The `Op::TransitionWithDiff` scoping has no test asserting the actor check is correctly suppressed on a non-diff field. The e2e walk above proves the behaviour, but unit-level regression tests would be cheaper to maintain. Recommend adding both in Phase 8. (m3) Reserved-column-name leaf collision (cycle-2 fresh-eye m1c2; carried from Phases 2/3/4/5/6) still not caught at install time. Phase 7 didn't address; deferrable to v0.2 per the plan's stated risk acceptance.
  - **Forward-compat for Phase 8:** DONE_WHEN #12 cross-store JOIN verified live (returns real `L001|G001` match). `tests/e2e.sh` README correspondence is Phase 8's job; nothing in Phase 7's surface conflicts. `--json` polish is unaffected (gate has no Records, just one List `options` which goes through the existing code path). The Op::Update bug (M1) will not bite the planned Phase 8 e2e per the README demo path, but if anyone adds an "update an existing gate's question" step it will surface — Phase 8 should be aware.
  - **DONE_WHEN:** #3 fully (multi-store coexistence with `gate` table beside `observations` in same DB), #9 (gate add returns G001 with `--task-ref L001`), #10 (human can answer), #11 (CLAUDECODE-detected ai_autonomous rejected with documented error format).
  - **Status:** Advance to Phase 8 — `EXECUTING_PHASE_8`.
